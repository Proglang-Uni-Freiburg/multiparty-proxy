...
from src.scribble.visit import visitor

from src.scribble.ast_scribble.globel import (
    globalprotocolblock,
    ...
    globalinterrupt,
    globalinterruptible)
#import ast_scribble.globaldo as globaldo 
# something weird is going on that doesn't allow this; it's to do with circular
# imports

from src.scribble import ast_scribble  # HACK: because (recursive) import ast_scribble.globaldo is a problem

from src.scribble.ast_scribble import (
    argument,
    argumentlist,
    messagesignature,
    parameter,
...
class NameSubstitutor(visitor.Visitor):
    ...
    def _visit_globalmessagetransfer(self, node):
        ...
        if util.get_node_type(msgnode) != constants.MESSAGE_SIGNATURE_NODE_TYPE:
            ...
            if msg in list(argmap.keys()):  # a param to substitute
            ...
            for pe in payload.getChildren():
                ...
                tmp4 = child.dupNode()
                if child.getText() in list(argmap.keys()):
        ...
        if src in list(rolemap.keys()):  # FIXME: factor out this routine
            val = rolemap[src]
            #token = tmp.getToken()
    ...
    def _visit_globaldo(self, node):
        ...
        scopenode = ast_scribble.globel.globaldo.get_scope_child(node)
        dup.addChild(scopenode.dupNode())

        #protocolnode = ast_scribble.globel.globaldo.getglobaldoProtocolChild(node)
        protocolnodes = ast_scribble.globel.globaldo.get_target_name_children(node)
        argnodes = argumentlist.get_argument_children(ast_scribble.globel.globaldo.get_argumentlist_child(node))
        roleinstantiations = roleinstantiationlist.get_roleinstantiation_children(ast_scribble.globel.globaldo.get_roleinstantiationlist_child(node))  # list of ROLEINSTANTIATION
        #packagenodes = ast_scribble.globel.globaldo.getglobaldoPackageChildren(node)

        #dup.addChild(protocolnode.dupNode())

        #dup.addChild(copy.deepcopy(node.getChild(1)))  # FIXME: handle argnodes "properly"?
        tmp = ast_scribble.globel.globaldo.get_argumentlist_child(node).dupNode()
        ...
        tmp = ast_scribble.globel.globaldo.get_roleinstantiationlist_child(node).dupNode()  # HACroleinstantiations node
