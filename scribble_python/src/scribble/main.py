...
from src.scribble.moduleloader import load_modules as load_modules
from src.scribble.memberloader import load_members as load_members
from src.scribble.visibilitybuilder import build_visibility as build_visibility

from src.scribble.visit.context import Context as Context
from src.scribble.visit.projector import Projector as Projector
from src.scribble.visit.scopeinserter import ScopeInserter as ScopeInserter
from src.scribble.visit.unfolder import Unfolder as Unfolder
from src.scribble.visit.wellformednesschecker import \
    WellformednessChecker as WellformednessChecker
from src.scribble.visit.reachabilitychecker import ReachabilityChecker as ReachabilityChecker
from src.scribble.visit.subprotocolcollector import \
...
def main(argv, otherArg=None):
    ...
    # parent/node) -- these fields a re initialised on entering the AST from the
    # root module node
...
def _insert_scopes(context_):
    for fullname, module in list(context_.get_modules().items()):
        si = ScopeInserter()
...
def _check_wellformedness(context_):
    for _, module_ in list(context_.get_modules().items()):
...
def _check_reachability(context_):
    for proto, lpd in list(context_.get_projections().items()): 
...
def _output_projections_to_modules(context_, target_globalname, localrole, dir):
    ...
    if target_globalname not in list(members.keys()):
    ...
    if len(argv) > 1:
        ...
        for arg in iter:
            if arg == '-ip' or arg == '-importpath':  # Factor out constants
                path = next(iter).split(';')
                args[_IMPORT_PATH].extend(path)
            elif arg == '-pp' or arg == '-payloadpath':
                path = next(iter).split(';')
                args[_PAYLOAD_TYPE_PATH].extend(path)
            elif arg == '-project':
                args[_PROJECT_PROTOCOL] = next(iter)
                args[_PROJECT_ROLE] = next(iter)
            elif arg == '-o':
                args[_PROJECT_DIR] = next(iter)
            else:
                if arg.startswith('-'):
